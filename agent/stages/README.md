...
- **LLMStage**：调用 LLM Provider 编排生成（支持多步工具调用、流式输出、UsageRecorder、延迟警告合并、防偷懒门禁、上下文压缩、工具审批/ToolDeferral 注入）。生成结果写入 `ActionReply` 并存入 Envelope KV `llm.result`。
...
- **NoteCaptureMiddleware**：中间件，在 LLM 生成回复后自动把回复文本捕获为 L0 笔记（`ActionNote`），补齐记忆捕获路径。
...
| `NoteCaptureMiddleware` | 回复笔记捕获中间件 |
...
| `note_capture.go` | `NoteCaptureMiddleware` |
...
`NoteCaptureMiddleware` 作为 `core.Stage` 包装器叠加在回复路径上：

```go
wrapped := stages.NoteCaptureMiddleware("exchange")(replyStage)
```
