...
func TestToolCallsDigest(t *testing.T) {
	...
	if toolCallsDigest(r1, nil) != toolCallsDigest(r2, nil) {
...
func TestToolCallsDigest_ExemptTools(t *testing.T) {
	...
	// 豁免集合不同时不应影响普通工具的 digest
	if got2 := toolCallsDigest(mixed, nil); got2 == "" {
		t.Error("without exempt set, digest should be non-empty")
	}
}
