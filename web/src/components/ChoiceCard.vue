...
// mode 同时认 'multiple'（前端归一后的值）与 'multi'（后端 user_choice 工具的枚举，
// 见 tools/user_choice.go 的 mode enum）。只认 'multiple' 会让直传原始 payload 的
// 场景把多选题静默降级成单选。
const mode = computed(() => {
  const m = String(props.payload?.mode || '')
  return (m === 'multiple' || m === 'multi') ? 'multiple' : 'single'
})
