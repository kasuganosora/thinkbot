...
func newBotService(p APIParams, eventBus outbound.EventBus, chatHistory *ChatHistoryService, permSvc *toolperm.Service, bindStage *identity.BindStage, bindSvc *identity.BindService) *BotService {
	tp := p.TP
	if tp == nil {
	...
	return NewBotService(p.DB, p.Store, p.BotMgr, p.Logger, tp, mp, eventBus,
		p.StatsRecorder, judgeSink, chatHistory, permSvc, bindStage, bindSvc)
}
