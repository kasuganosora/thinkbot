...
func (s *BotService) StartBot(ctx context.Context, id string) error {
	...
	// 复合召回：L3 画像 → L1 蒸馏 → memory_entries 原始笔记。高价值源排最前，
	// 确保 Snapshot 字符预算截断时画像/项目事实不被丢弃；按内容去重避免重复计权。
	...
	if dreamBundle != nil {
		...
		dreamBundle.Manager.SetOnBotProfileUpdated(func(botID string, result *memory.BotProfileResult) {
			...
			if result.Confidence < memory.MinProfileWriteConfidence {
				s.logger.Infow("skip low-confidence bot profile from dreaming (keep SOUL seed)",
					"bot_id", botID, "confidence", result.Confidence)
				return
			}
