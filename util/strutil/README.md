...
1. 去除首尾空白与 BOM，剥掉 markdown 代码块标记（` ```json ` / ` ``` `）
2. 直接解析整个字符串
3. 括号配平扫描定位最外层 JSON 对象 `{...}`，再扫描最外层数组 `[...]`
   （字符串字面量内的括号会被忽略，比"第一个 `{` 到最后一个 `}`"更稳健）
4. 容错清理后重试上述 2、3 步：
   - 将字符串字面量内的裸控制字符转义为 `\n` / `\r` / `\t`，其余控制字符丢弃
   - 删除对象/数组的尾随逗号（`,}` / `,]`）
5. 全部失败时返回原始 `json.Unmarshal` 错误

容错清理专门针对 LLM 常见的退化输出（例如在字符串值里直接输出换行导致非法 JSON）。
...
- `ExtractJSON` 专为 LLM 输出场景设计（容错 markdown 包裹、说明文字包裹、字符串内裸换行、尾随逗号）
