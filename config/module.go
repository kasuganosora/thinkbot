...
func NotifyMetaSpecs() []MetaSpec {
	return []MetaSpec{
		...
		{Key: KeyNotifyDefaultMode, Category: "Notify", Description: "默认模式 bot | raw（默认 bot：由 bot 以自身人格/记忆/主人会话上下文整理后发送；persona 为 bot 的旧别名）。per-bot：bot.<id>.notify.mode"},
