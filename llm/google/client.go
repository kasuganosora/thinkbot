...
// Client 是 Google Gemini API 客户端，封装 util/http.Client。
type Client struct {
...
func New(opts ...Option) *Client {
	...
	// 构建 httputil.Client options
	httpOpts := []httputil.Option{
		httputil.WithBaseURL(cfg.baseURL),
		httputil.WithHeader("x-goog-api-key", cfg.apiKey),
	}
	if cfg.timeout > 0 {
	...
	if cfg.proxy != "" {
		httpOpts = append(httpOpts, httputil.WithProxy(cfg.proxy))
	}
...
// modelPath 构建模型 API 路径。
// 例: modelPath("gemini-2.5-flash", "generateContent") => "/v1beta/models/gemini-2.5-flash:generateContent"
func modelPath(model, method string) string {
	return "/v1beta/models/" + model + ":" + method
}

// newRequest 创建一个已携带认证头的请求。
