...
// StreamPartType identifies the kind of streaming chunk.
...
// StreamPart is implemented by all streaming chunk types.
...
// --- Tool call stream part (complete) ---
...
//
// Steps and Messages are populated during multi-step stream consumption and
// are safe to read after Stream is fully consumed (i.e., after a for-range loop exits).
...
func (sr *StreamResult) ToResult() (*GenerateResult, error) {
	...
	for part := range sr.Stream {
		...
		case *FinishStepPart:
			...
			// Capture as fallback in case FinishPart never arrives (stream error)
